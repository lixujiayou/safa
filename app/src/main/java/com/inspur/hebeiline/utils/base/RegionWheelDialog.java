...
import com.inspur.hebeiline.utils.wheel.OnWheelChangedListener;
import com.inspur.hebeiline.utils.wheel.WheelView;
import com.inspur.hebeiline.utils.wheel.adapters.ArrayWheelAdapter;
...
public class RegionWheelDialog extends Activity implements OnWheelChangedListener {
    ...
    private void initDatas() {
        SPUtil spUtil = new SPUtil(RegionWheelDialog.this, "adress");
        String cProvince = spUtil.getString("cProvince", null);
        String cCity = spUtil.getString("cCity", null);
        String cDistrict = spUtil.getString("cDistrict", null);
        Log.d("qqqqqqq",cProvince+"省市区"+cCity+"，"+cDistrict);
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                if (cProvince != null) {
                    if (cProvince.startsWith(province)) {
                        indexCurProvice = i;
                    }
                }
                ...
                for (int j = 0; j < jsonCs.length(); j++) {
                    ...
                    if (cDistrict != null) {
                        ...
                        if (cDistrict.startsWith(city)) {
                            Log.d("qqqqq",j+"循环到哪个市了呢"+city);
                            indexCurCity = j;
                        }
                    }
                    ...
                    for (int k = 0; k < jsonAreas.length(); k++) {
                        ...
                        if (cDistrict != null && !"".equalsIgnoreCase(cDistrict)) {
                            if (cDistrict.startsWith(area)) {
                                indexCurArea = k;
                            }
                        }
                    }
