...
public class GetMineModel extends BaseViewModel {
    private boolean isRequesting = false;

    public GetMineModel(Application application) {
        super(application);
    }

    private MutableLiveData<LXMineEntity> roundSiteList;
    private Context mContext;


    public MutableLiveData<LXMineEntity> getCurrentData(Context context) {
     ...
     */
    public void getMine(MallRequest mRequest,String Mounth,String resLevel) {
        ...
        String url = AllUrl.mainUrl + AllUrl.getXJRecordByUser+"?month="+Mounth+"&resLevel="+resLevel+"&userId=T400004238&countyId="+cId;
        Log.d("qqqq","查询待办=="+url);
        showProgressDialog(mContext,"正在查询...");
        mRequest.getMineInfo(map
                , url)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<LXMineEntity>() {
                    @Override
                    ...
                    @Override
                    public void onNext(LXMineEntity roundSiteEntity) {
